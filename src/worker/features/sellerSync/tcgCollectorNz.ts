...
const BASE_URL = 'https://tcgcollectornz.com';
...
export class TcgCollectorNzAdapter implements SellerAdapter {
  readonly key = 'tcgcollector-shopify';
...
async function fetchCollectionProducts(page: number): Promise<ShopifyProduct[]> {
  ...
  if (!response.ok) {
    throw new Error(`TCG Collector NZ collection request failed with status ${response.status}`);
  }
